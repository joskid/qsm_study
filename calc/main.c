...
int main() 
{
   Calc_ctor();            /* We instantiate the calculator object */
                           // inside this CTOR we will call:  QHsm_ctor( &me->super, (QStateHandler)&Calc_initial );
                           // we can do this because the source calc.c is considered application code.  Inside
                           // the QHsm_ctor there will be a call to Calc_initial.  Inside Calc_inital we will call:
                           // Q_TRAN( &Calc_on ); Setting the target state for
                           // for when QHsm_init is called

   printf( "sizeof QSignal = %lu bytes\n", sizeof( QSignal ) );
   ...
   QHsm_init( the_calc, (QEvt *)0 );         /* trigger "THE" initial transition */
   ...
   for( ;; )                                  /* event loop */
   {
      ...
      printf( ": " );
      fflush( stdout );
      e.key_code = (uint8_t)getchar();        /* BLOCK here till: we get a char with echo (key_code) and inject into the event 'e'   */

      switch( e.key_code )                    // look at the key code and flesh out  'e', based on it, then send (dispatch it)
      {
      ...
      if( ( (QEvt *)&e )->sig != 0 ) QHsm_dispatch( the_calc, (QEvt *)&e );  /* valid event generated?  dispatch event */
   }
   return 0;
}
