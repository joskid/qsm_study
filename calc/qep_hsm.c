...
void QHsm_dispatch_( QHsm *const me, QEvt const *const e ) 
{
   ...
   if( r >= ( QState )Q_RET_TRAN ) 
   {
      ...
      /* retrace the entry path in reverse( desired ) order... */
      for( ; ip >= ( int_fast8_t )0; --ip )  QEP_ENTER_( path[ip] );  /* enter path[ip] */
...
static int_fast8_t QHsm_tran_( QHsm * const me, QStateHandler path[QHSM_MAX_NEST_DEPTH_] )
{
   ...
   else 
   {
      ...
      if( s == t )  ip = ( int_fast8_t )0; /* enter the target */
      else 
      {
         ...
         else 
         {
            ...
            if( me->temp.fun == path[0] )  QEP_EXIT_( s ); /* exit the source */
            else 
            {
               ...
               */
               ...
               while( r == ( QState )Q_RET_SUPER ) 
               {
                  ...
                  else  r = QEP_TRIG_( me->temp.fun, QEP_EMPTY_SIG_ );
               }
               ...
               if( iq == ( int_fast8_t )0 ) 
               {
                  ...
                  */
                  ...
                  do 
                  {
                     ...
                     else  --iq; /* try lower superstate of target */
